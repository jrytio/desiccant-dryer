...
Opened on GitHub so far: #34 covers B-20 and J-09 through test T-B20 and seeds the config-lint CI job (T-F15, T-J04); #36 builds the host runner for the logic, detect and infer cases (T-B01, T-C07 first); #40 adds the nine plant-model fault knobs that host cases need, unblocking T-B02 and T-B06. Record each new GitHub issue number here when it is opened.
