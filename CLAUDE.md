...
(window on the Mac). `packages/screen-mirror.yaml` (device builds only) serves the panel's frame buffer as `/screen.bmp` through the local component `esphome/components/screen_mirror`. `desiccant-dryer.yaml`, `desiccant-dryer-virtual.yaml`
